...
const addLearningMaterial = asyncHandler(async(req, res) => {
  const { title, description, type } = req.body;
  const { courseId, teacherId } = req.params;
  const loggedTeacher = req.teacher;

  if (!title || !type) {
    throw new ApiError(400, "Title and type are required fields");
  }

  // Verify that the logged in teacher matches the teacher ID in the URL
  if (loggedTeacher._id.toString() !== teacherId) {
    throw new ApiError(403, "Not authorized: Teacher ID mismatch");
  }

  // Check if the teacher is enrolled for this course
  ...
  // Handle file upload for different types of materials
  if (type === 'link') {
    // For links, directly use the URL provided in the request body
    if (!req.body.url) {
      throw new ApiError(400, "URL is required for link type materials");
    }
    url = req.body.url;
  } else {
    // For files (pdf, image, audio, ppt), upload to Cloudinary
    if (!req.file) {
      throw new ApiError(400, "File is required for this material type");
    }
  ...
  // Add the learning material to the course
  ...
  if (!updatedCourse) {
    throw new ApiError(500, "Failed to add learning material");
  }

  return res
    .status(200)
    .json(new ApiResponse(200, updatedCourse.learningMaterials, "Learning material added successfully"));
});

const getLearningMaterials = asyncHandler(async(req, res) => {
  const { courseId } = req.params;

  // Find the course
  const foundCourse = await course.findById(courseId).select("learningMaterials");
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, foundCourse.learningMaterials, "Learning materials retrieved successfully"));
});

const deleteLearningMaterial = asyncHandler(async(req, res) => {
  const { courseId, materialId, teacherId } = req.params;
  const loggedTeacher = req.teacher;
  ...
  if (!enrolledCourse) {
    throw new ApiError(403, "Not authorized to delete materials from this course");
  }

  // Find and remove the learning material
  const updatedCourse = await course.findByIdAndUpdate(
    courseId,
    {
      $pull: {
        learningMaterials: { _id: materialId }
      }
    },
    { new: true }
  );
  ...
  if (!updatedCourse) {
    throw new ApiError(500, "Failed to delete learning material");
  }

  return res
    .status(200)
    .json(new ApiResponse(200, updatedCourse.learningMaterials, "Learning material deleted successfully"));
});
