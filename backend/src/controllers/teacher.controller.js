...
const addTeacherDetails = asyncHandler(async(req,res)=>{
    ...
    if([Phone, Address, Experience, SecondarySchool, HigherSchool,UGcollege, PGcollege, SecondaryMarks, HigherMarks, UGmarks, PGmarks].some((field)=> 
...
const teacherdocuments = asyncHandler(async(req, res) => {
    try {
        ...
        if (!teacherDocs) {
            throw new ApiError(404, "No teacher documents found");
        }

        return res
            .status(200)
            .json(new ApiResponse(200, teacherDocs, "Teacher documents fetched successfully"));
    } catch (error) {
        console.error("Error fetching teacher documents:", error);
        throw new ApiError(error.statusCode || 500, error.message || "Failed to fetch teacher documents");
    }
});
